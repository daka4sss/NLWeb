...
def get_item_name(item: Dict[str, Any]) -> str:
    ...
    if len(parts) > 1:
        ...
        name = " ".join(
            word.capitalize() for word in longest_part.replace("-", " ").split()
        )
        return name
...
def prepare_documents_from_json(
    url: str, json_data: str, site: str
...
# Note: This function is maintained for backward compatibility
async def upload_batch_to_db(
    client, db_type, documents, batch_idx, total_batches, endpoint_name=None
):
    """
    ...
    try:
        print(
            f"Uploading batch {batch_idx+1} of {total_batches} ({len(documents)} documents)"
        )
