...
def is_podcast_feed(channel: ET.Element) -> bool:
    """Heuristically determine if an RSS channel represents a podcast feed."""
    # Look for iTunes or podcast specific elements
    for ns_prefix, ns_uri in NAMESPACES.items():
        if ns_prefix == "itunes":
            if channel.find(f".//{{{ns_uri}}}duration") is not None:
                return True
    # Check a few items for audio enclosures
...
def parse_rss_2_0(
    ...
    podcast_feed = is_podcast_feed(channel)
    ...
    # Create basic feed series schema
    series_type = "PodcastSeries" if podcast_feed else "DataFeed"
    podcast_series = {
        "@type": series_type,
        "name": podcast_title,
    ...
    for item in channel.findall("item"):
        try:
            ...
            episode_type = "PodcastEpisode" if podcast_feed else "Article"
            episode = {
                "@type": episode_type,
                "name": title,
            ...
            enclosure = item.find("enclosure")
            if podcast_feed and enclosure is not None:
            ...
            for ns_prefix, ns_uri in NAMESPACES.items():
                if podcast_feed and ns_prefix == "itunes":
                    # Duration
...
def parse_atom(
    ...
    # Determine if this feed is a podcast
    podcast_feed = False
